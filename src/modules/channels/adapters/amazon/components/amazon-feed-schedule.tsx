...
/**
 * Amazon Feed Schedule Component
 *
 * Story 17.2 - AC1: Feed Schedule Configuration
 * Story 17.2 - AC4: Manual Feed Trigger
 *
 * Allows publishers to configure automated ONIX feed delivery schedules to Amazon.
 * Follows IngramFeedSchedule pattern from Story 16.2.
 */
...
import { saveAmazonSchedule, triggerAmazonFeed } from "../actions";
import type { AmazonSchedule } from "../types";
...
interface AmazonFeedScheduleProps {
  currentSchedule: AmazonSchedule | null;
  isConnected: boolean;
}

export function AmazonFeedSchedule({
  currentSchedule,
  isConnected,
}: AmazonFeedScheduleProps) {
  ...
  async function handleSubmit(data: ScheduleFormData) {
    startTransition(async () => {
      const result = await saveAmazonSchedule(data);
      if (result.success) {
  ...
  async function handleManualTrigger() {
    ...
    try {
      const result = await triggerAmazonFeed();
      if (result.success) {
        toast.success(result.message);
      } else {
        toast.error(result.message);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Configure automatic ONIX feed delivery to Amazon
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {!isConnected ? (
          <p className="text-muted-foreground">
            Connect your Amazon account to configure feed scheduling.
          </p>
        ) : (
          <>
            <Form {...form}>
            ...
            <div className="border-t pt-4">
              ...
              <p className="text-sm text-muted-foreground mb-3">
                Generate and send an ONIX feed to Amazon immediately.
              </p>
              <Button
                variant="outline"
