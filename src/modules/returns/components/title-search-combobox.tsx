...
/**
 * Title Search Combobox Component
 *
 * Searchable dropdown for selecting titles in returns entry form.
 ...
 * AC 3: Format dropdown filtered by selected title
 */
...
import { searchTitlesForReturnsAction } from "../actions";
import type { SelectedTitleForReturn, TitleForReturnSelect } from "../types";

interface TitleSearchComboboxProps {
  /** Currently selected title */
  value: SelectedTitleForReturn | null;
  /** Callback when title is selected */
  onSelect: (title: SelectedTitleForReturn | null) => void;
...
export function TitleSearchCombobox({
  ...
  const [titles, setTitles] = React.useState<TitleForReturnSelect[]>([]);
  const [isPending, startTransition] = useTransition();
  const debounceRef = React.useRef<NodeJS.Timeout | null>(null);

  // Debounced search effect (AC 2: 300ms debounce)
  React.useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      startTransition(async () => {
        const result = await searchTitlesForReturnsAction(searchQuery);
        if (result.success) {
  ...
  const handleSelect = (title: TitleForReturnSelect) => {
    onSelect({
      id: title.id,
      title: title.title,
  ...
  // Format badges showing available formats (AC 2)
  const formatBadges = (title: TitleForReturnSelect) => {
    const badges: string[] = [];
