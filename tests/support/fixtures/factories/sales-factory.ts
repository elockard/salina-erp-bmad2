...
import type { Title, TitleFactory } from "./title-factory";
...
export class SalesFactory {
  ...
  async createSale(
    ...
    const quantity =
      overrides.quantity || faker.number.int({ min: 1, max: 500 });
    const unit_price =
      overrides.unit_price ||
      parseFloat(faker.commerce.price({ min: 10, max: 50 }));
    ...
    const sale: Omit<Sale, "id"> = {
      ...
      format:
        overrides.format ||
        faker.helpers.arrayElement(["physical", "ebook", "audiobook"]),
      quantity,
      unit_price,
      total_amount,
      sale_date:
