...
async function applyMigration() {
  ...
  try {
    ...
    `;
    console.log(`  - Titles with eisbn but no isbn: ${titlesWithEisbnNoIsbn[0].count}`);

    console.log('\nApplying migration...\n');

    // Read and execute the migration
    const migrationSQL = readFileSync('./drizzle/migrations/0014_remove_isbn_type_distinction.sql', 'utf8');
    ...
    const prefixTypeCountAfter = await sql`
      SELECT COUNT(*) as count FROM isbn_prefixes WHERE type IS NOT NULL;
    `;
    console.log(`  - ISBN Prefixes with type set: ${prefixTypeCountAfter[0].count}`);
